...
import { SearchIcon, DocumentIcon, CubeIcon, ScaleIcon, InfoIcon, ArrowLeftIcon, ChatBubbleIcon, ArrowUpIcon, ArrowDownIcon } from './components/IconComponents';
            ...
            {isDetailLoading ? (
                ...
                <main className="grid grid-cols-1 md:grid-cols-12 gap-4 h-[calc(100vh-104px)]">
                    ...
                    <div className="md:col-span-5 flex flex-col h-full bg-slate-800/50 rounded-lg border border-slate-700 overflow-hidden">
                        {selectedMawb ? (<> <div className="p-4 border-b border-slate-700 flex-shrink-0"><div className="flex justify-between items-start"><div><h2 className="text-lg font-semibold text-white">{selectedMawb.awb_number}</h2><p className="text-sm text-slate-400">{selectedMawb.nature_of_goods}</p></div></div></div><div className="border-b border-slate-700 px-2 flex-shrink-0"><nav className="flex space-x-2"><button onClick={() => setActiveTab('details')} className={`px-3 py-2 text-sm font-medium rounded-t-md ${activeTab === 'details' ? 'border-b-2 border-cyan-400 text-white' : 'text-slate-400 hover:text-white'}`}>Details</button><button onClick={() => setActiveTab('ulds')} className={`px-3 py-2 text-sm font-medium rounded-t-md ${activeTab === 'ulds' ? 'border-b-2 border-cyan-400 text-white' : 'text-slate-400 hover:text-white'}`}>ULDs ({(selectedMawb.uld_contents || []).length})</button><button onClick={() => setActiveTab('hawbs')} className={`px-3 py-2 text-sm font-medium rounded-t-md ${activeTab === 'hawbs' ? 'border-b-2 border-cyan-400 text-white' : 'text-slate-400 hover:text-white'}`}>HAWBs ({(selectedMawb.house_shipments || []).length})</button></nav></div><div className="flex-grow overflow-y-auto p-4">{activeTab === 'details' && (<div className="space-y-3 text-sm"><div className="flex items-center"><CubeIcon className="w-4 h-4 mr-2 text-slate-500"/><strong>Pieces:</strong><span className="ml-2">{selectedMawb.pieces}</span></div><div className="flex items-center"><ScaleIcon className="w-4 h-4 mr-2 text-slate-500"/><strong>Weight:</strong><span className="ml-2">{selectedMawb.weight.value.toLocaleString()} {selectedMawb.weight.unit}</span></div><div className="flex items-center"><InfoIcon className="w-4 h-4 mr-2 text-slate-500"/><strong>SHC:</strong><span className="ml-2 flex flex-wrap gap-1">{selectedMawb.special_handling_codes.map(code => <span key={code} className="bg-slate-700 text-xs px-2 py-1 rounded-full">{code}</span>)}</span></div>{selectedMawb.storage_instructions && <div className="text-amber-300 bg-amber-900/50 p-2 rounded-md"><strong>Storage:</strong> {selectedMawb.storage_instructions}</div>}</div>)}{activeTab === 'ulds' && (<div className="space-y-1">{(selectedMawb.uld_contents || []).map((uld, idx) => ( <div key={`${uld.uld_id}-${idx}`} onClick={() => handleSelectItem(uld)} className={`p-2 rounded-md cursor-pointer transition-colors flex justify-between items-center ${selectedItem === uld ? 'bg-slate-600' : 'hover:bg-slate-700/50'}`}><div><p className="font-mono text-sm">{uld.uld_id}</p></div><div className="text-right text-xs"><p>{uld.pieces} pcs</p><p>{uld.weight.value.toLocaleString()} kg</p></div></div>))}</div>)}{activeTab === 'hawbs' && (<div className="space-y-1">{(selectedMawb.house_shipments || []).map((hawb, idx) => ( <div key={`${hawb.hawb_number}-${idx}`} onClick={() => handleSelectItem(hawb)} className={`p-2 rounded-md cursor-pointer transition-colors ${selectedItem === hawb ? 'bg-slate-600' : 'hover:bg-slate-700/50'}`}><p className="font-mono text-sm">{hawb.hawb_number}</p><p className="text-xs opacity-70">{hawb.customer} to {hawb.destination}</p></div>))}</div>)}</div></>) : (<div className="flex items-center justify-center h-full text-slate-500">Select a MAWB to see details.</div>)}
                    </div>
